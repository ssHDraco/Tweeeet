...
class TweetListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if requested_user:
            ...
            im_following = self.request.user.profile.get_following() # maybe none
            qs1 = Tweet.objects.filter(user__in=im_following)
            qs2 = Tweet.objects.filter(user=self.request.user)
            qs = (qs1 | qs2).order_by("-timestamp") # qs = Tweet.objects.filter(Q(user__in=im_following) | Q(user__exact=self.request.user)).order_by("-timestamp")
